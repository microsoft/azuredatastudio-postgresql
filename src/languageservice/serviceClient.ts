         ...
         return new Promise<ServerInitializationResult>( (resolve, reject) => {
            ...
            if (!platformInfo.isValidRuntime()) {
                 ...
                 if (platformInfo.isLinux()) {
                    this._logger.appendLine(SharedConstants.usingDefaultPlatformMessage);
                    platformInfo.runtimeId = Runtime.Linux_64;
                }

                let ignoreWarning: boolean = this._config.getWorkspaceConfig(SharedConstants.ignorePlatformWarning, false);
                if (!ignoreWarning) {
                    this._vscodeWrapper.showErrorMessage(
                        SharedConstants.unsupportedPlatformErrorMessage,
                        SharedConstants.neverShowAgain)
                    .then(action => {
                        if (action === SharedConstants.neverShowAgain) {
                            this._config.updateWorkspaceConfig(SharedConstants.ignorePlatformWarning, true);
                        }
                    });
                }
        ...
        return new Promise<boolean>((resolve, reject) => {
            this._client.sendRequest(VersionRequest.type, undefined).then((result) => {
                 ...
                 if (result === undefined || !result.startsWith(SqlToolsServiceClient._constants.serviceCompatibleVersion)) {
                     Utils.showErrorMsg(SharedConstants.serviceNotCompatibleError, SqlToolsServiceClient._constants.extensionName);
                     Utils.logDebug(SharedConstants.serviceNotCompatibleError, SqlToolsServiceClient._constants.extensionConfigSectionName);
                     resolve(false);
                 } else {
                     resolve(true);
                 }
