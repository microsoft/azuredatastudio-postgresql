...
export async function checkProjectVersion(minRequiredSDK: string, maxRequiredSDK: string, projects: string[], commandObserver: CommandObserver) : Promise<string[]> {
	...
	return Promise.resolve(unsupportedProjectsMap.map(p => p.path));
}
...
function readXML(project: string, fileText: string, commandObserver: CommandObserver): ProjectInformation {
    ...
    parser.parseString(fileText, function(err, result) {
        if (result && result.Project && result.Project.$ && result.Project.$.Sdk) {
            ...
            if (splitArray.length === 2 && splitArray[0].replace(/\s/g, '').toUpperCase() === sdkName.toUpperCase()) {
                version = splitArray[1];
            }
        }
        else if(result && result.Project && result.Project.Sdk
            && result.Project.Sdk[0].$ && result.Project.Sdk[0].$.Name && result.Project.Sdk[0].$.Version) {
                referenceType = sdkReference.Element;
                if (result.Project.Sdk[0].$.Name.toUpperCase() === sdkName.toUpperCase()) {
                    version = result.Project.Sdk[0].$.Version;
                }
        }
    });
...
function promptToUpdateVersion(unsupportedProjects: Array<ProjectInformation>, maxRequiredSDK: string, commandObserver: CommandObserver) {
    ...
    let installItem = Constants.updateProjectButtonText;
    vscode.window
